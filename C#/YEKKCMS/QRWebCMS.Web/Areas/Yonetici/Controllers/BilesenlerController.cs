...
namespace QRWebCMS.Web.Areas.Yonetici.Controllers
{
    ...
    public class BilesenlerController : Controller
    {
        ...
        public ActionResult MenuSayfa(int id)
        {
            ...
            TempData["EtkinSayfaListesi"] = sayfaListesi;
            TempData["SayfaListesi"] = sayfa;
            TempData["MenuID"] = id;
        ...
        public ActionResult MenuSayfaTemizle(int? id)
        {
            if (id == null)
                return RedirectToAction(nameof(Menu));
            ...
            if(menuListe.Count > 0)
            {
                int durum = Tanimlamalar.menuListeIslemleri.Sil("MenuID = " + id);

                if (durum <= 0)
                    throw new Exception();
            }

            return RedirectToAction(nameof(MenuSayfa), new { id });
        }
        ...
        public ActionResult SayfaIcerik(int id)
        {
            ...
            List<GaleriKategori> galeriListesi = new List<GaleriKategori>();
            List<Icerik> icerikListesi = new List<Icerik>();
            List<UrunKategori> urunListesi = new List<UrunKategori>();
            List<SlideKategori> slideListesi = new List<SlideKategori>();

            foreach (var item in sayfaListe)
            {
                switch (item.NitelikTablosu)
                {
                    case "Icerik": icerikListesi.Add(Tanimlamalar.icerikIslemleri.Bul("IcerikID = " + item.NitelikID)); break;
                    case "GaleriKategori": galeriListesi.Add(Tanimlamalar.galeriKategoriIslemleri.Bul("GaleriKategoriID = " + item.NitelikID)); break;
                    case "SlideKategori": slideListesi.Add(Tanimlamalar.slideKategoriIslemleri.Bul("SlideKategoriID = " + item.NitelikID)); break;
                    case "UrunKategori": urunListesi.Add(Tanimlamalar.urunKategoriIslemleri.Bul("UrunKategoriID = " + item.NitelikID)); break;
                }
            }

            TempData["Icerik"] = Tanimlamalar.icerikIslemleri.VeriGetir("Etkin = 1");
            TempData["Galeri"] = Tanimlamalar.galeriKategoriIslemleri.VeriGetir("Etkin = 1");
            TempData["Slide"] = Tanimlamalar.slideKategoriIslemleri.VeriGetir("Etkin = 1");
            TempData["Urun"] = Tanimlamalar.urunKategoriIslemleri.VeriGetir("Etkin = 1");
            TempData["SayfaID"] = id;

            TempData["EtkinIcerik"] = icerikListesi;
            TempData["EtkinGaleri"] = galeriListesi;
            TempData["EtkinSlide"] = slideListesi;
            TempData["EtkinUrun"] = urunListesi;
        ...
        public ActionResult SayfaIcerikTemizle(int? id)
        {
            if (id == null)
                return RedirectToAction(nameof(Sayfa));
            ...
            {
                int durum = Tanimlamalar.sayfaListeIslemleri.Sil("SayfaID = " + id);

                if (durum <= 0)
                    throw new Exception("Silme işlemi gerçekleştirilemedi");
            }

            return RedirectToAction(nameof(SayfaIcerik), new { id });
        }

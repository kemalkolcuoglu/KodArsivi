...
namespace QRWebCMS.Web.Controllers
{
    ...
    public class SayfaController : Controller
    {
        ...
        public ActionResult OnlineKayit()
        {
            return View();
        }

        [HttpPost]
        public ActionResult OnlineKayit(Kayit kayit)
        {
            if (ModelState.IsValid)
            {
                kayit.KayitGonderimTarihi = DateTime.Now;
                int durum = Tanimlamalar.kayitIslemleri.Ekle(kayit);
                if (durum > 0)
                {
            ...
            ViewBag.Hata = "Kayıt bilgileriniz alınamadı. Lütfen bilgileri kontrol ederek tekrar deneyiniz.";
            return View(kayit);
        }
    }
}